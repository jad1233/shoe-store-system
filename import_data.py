...
def import_products(conn):
    ...
    for _, row in df.iterrows():
        ...
        manufacturer_id = conn.execute("SELECT id FROM manufacturers WHERE name=?", (manufacturer,)).fetchone()[0]
        supplier_id = conn.execute("SELECT id FROM suppliers WHERE name=?", (supplier,)).fetchone()[0]
